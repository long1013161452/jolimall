//
//  XZZCheckOutShowAddressInforView.m
//  XZZJoLiMall
//
...
@interface XZZCheckOutShowAddressInforView ()
...
@implementation XZZCheckOutShowAddressInforView

+ (instancetype)allocInit
{
    XZZCheckOutShowAddressInforView * view = [super allocInit];
    [view addView];
    return view;
}
...
- (void)addView
{
    ...
    [self.nameLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.right.equalTo(weak_backView).offset(-40);
    ...
    [button addTarget:self action:@selector(clickOnAddressView) forControlEvents:(UIControlEventTouchUpInside)];
...
{
    !self.changeAddress?:self.changeAddress();
}
