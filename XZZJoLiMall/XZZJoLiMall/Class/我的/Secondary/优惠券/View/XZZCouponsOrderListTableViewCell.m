//
//  XZZCouponsOrderListTableViewCell.m
//  XZZJoLiMall
//
...
@interface XZZCouponsOrderListTableViewCell ()
/**
 * 背景图
 */
 ...
 * 使用按钮  button
 */
@property (nonatomic, strong)UIButton * useButtonTwo;
/**
 * <#expression#>
 */
@property (nonatomic, strong)UILabel * priceLanel;
@end

@implementation XZZCouponsOrderListTableViewCell
...
- (void)addView
{
    ...
    self.priceLanel = [UILabel labelWithFrame:CGRectMake(0, 0, 0, 0) backColor:nil textColor:kColor(0xffffff) textFont:15 textAlignment:(NSTextAlignmentCenter) tag:1];
    self.priceLanel.font = textFont_bold(15);
    self.priceLanel.numberOfLines = 2;
    [self addSubview:self.priceLanel];
    [self.priceLanel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(@10);
    ...
    [self.useButton mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerX.equalTo(wSelf.priceLanel);
        make.top.equalTo(wSelf.priceLanel.mas_bottom);
        make.bottom.equalTo(wSelf).offset(-12);
        make.right.equalTo(wSelf.backImageView1).offset(-12);
        make.width.equalTo(@(width - 20));
        make.height.equalTo(@24);
    }];
    
    self.useButtonTwo = [UIButton allocInitWithTitle:@"Use it" color:kColor(0x000000) selectedTitle:@"Use it" selectedColor:kColor(0x000000) font:13];
    self.useButtonTwo.layer.cornerRadius = 12;
    self.useButtonTwo.layer.masksToBounds = YES;
    self.useButtonTwo.backgroundColor = kColor(0xfff8f5);
    [self.useButtonTwo addTarget:self action:@selector(clickOnButton) forControlEvents:(UIControlEventTouchUpInside)];
    [self addSubview:self.useButtonTwo];
    [self.useButtonTwo mas_makeConstraints:^(MASConstraintMaker *make) {
...
- (void)setCouponsInfor:(XZZCouponsInfor *)couponsInfor
{
    ...
    if (couponsInfor.couponDiscount > 0) {
        self.priceLanel.text = [NSString stringWithFormat:@"Save $%.2f", couponsInfor.couponDiscount];
        self.useButton.hidden = NO;
        self.useButtonTwo.hidden = YES;
    }else{
        self.priceLanel.text = @"";
        if (self.couponAvailable && !couponsInfor.orderCanUse) {
            self.useButtonTwo.hidden = YES;
        }else{
            self.useButtonTwo.hidden = NO;
        }
        self.useButton.hidden = YES;
    }
}
